...
* Usage
* Concepts
*** Setting up Python2 virtual environment
...
*** Using unicode
-setting strings with unicode
#+BEGIN_EXAMPLE
unicode_string = u'abc_\u03a0\u03a3\u03a9'
#+END_EXAMPLE

*** Using re
- print all words that begin with "a"
#+BEGIN_EXAMPLE
import re

re_string = 'a[a-z]*'

some_string = "the animals in the asylum are anorexic"
...
for match in re.findall(re_string, some_string):
    print("MATCH -> ", match)
#+END_EXAMPLE
compiled patterns can cut down on execution time

- the four most used regular expressions
  - [[https://docs.python.org/2/library/re.html][findall()]]
  - [[https://docs.python.org/2/library/re.html][finditer()]]
  - [[https://docs.python.org/2/library/re.html][match()]]
  - [[https://docs.python.org/2/library/re.html][search()]]
...
print(infile.read())
#+END_EXAMPLE
this will output the file implementing file special characters

- writing to a file
#+BEGIN_EXAMPLE
outputfile = open("foo.txt", "w")

outputfile.write("This will be added to the foo.txt file\n")
outputfile.close()
#+END_EXAMPLE

- reading files
#+BEGIN_EXAMPLE
files_r = open("foo.txt", "r"
#+END_EXAMPLE
this will output the file with special characters printed

- there are 3 read functions
  - [[https://docs.python.org/2/tutorial/inputoutput.html][read()]]
  - [[https://docs.python.org/2/tutorial/inputoutput.html][readline()]]
  - [[https://docs.python.org/2/tutorial/inputoutput.html][readlines()]]
    
*** Stdout Stdin Stderr
- [[https://docs.python.org/2/library/sys.html][sys]]
k
*** Apache change the virtual host root
#+BEGIN_EXAMPLE
#!/bin/usr/env python
...
vhsot_end = re.compile(r'</VirtualHost')
...
def replace_docroot(conf_string, vhost, new_docroot):
    ...
    for line in conf_file:
        curr_vhost = vhost_start.search(line)
        in_vhost = True
